using System;
using System.Collections;
using UnityEngine;
using UnityEngine.Events;

namespace MVC.Base.Runtime.Abstract.Injectable.Provider
{
    public interface IUpdateProvider
    {
        void AddUpdate(UnityAction action);
        void RemoveUpdate(UnityAction action);

        void AddFixedUpdate(UnityAction action);
        void RemoveFixedUpdate(UnityAction action);

        void AddLateUpdate(UnityAction action);
        void RemoveLateUpdate(UnityAction action);

        Coroutine StartCoroutine(IEnumerator coroutine);
        void StopCoroutine(Coroutine coroutine);
        void StopAllCoroutines();
        
        IScheduledUpdateAction AddUpdateScheduledAction(float remainingTime, UnityAction onUpdate, Action onComplete = null);
        void RemoveUpdateScheduledAction(IScheduledUpdateAction scheduledUpdateAction);
    }
}